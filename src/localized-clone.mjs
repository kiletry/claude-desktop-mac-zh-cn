...
export function patchNativeMenuLocale(source) {
  ...
  if (count === 0) {
    const modernMatches = [...source.matchAll(/async function ([A-Za-z_$][A-Za-z0-9_$]*)\(\)\{let e=await ([A-Za-z_$][A-Za-z0-9_$]*)\(\);return ([A-Za-z_$][A-Za-z0-9_$]*)\.Menu\.buildFromTemplate\(e\)\}/g)];
    if (modernMatches.length !== 1) {
      throw new CompatibilityError('Expected exactly one native menu locale target, found 0.');
    }
    const [, menuBuilder, templateBuilder, menuNamespace] = modernMatches[0];
    ...
    return source.replace(modernMatches[0][0], modernPatch.replace("return o.Menu.buildFromTemplate", `return ${menuNamespace}.Menu.buildFromTemplate`));
  }
  if (count !== 1) {
  ...
  const nativeMenuPatch = [
    ...
    'const r={undo:`撤销`,redo:`重做`,cut:`剪切`,copy:`复制`,paste:`粘贴`,pasteAndMatchStyle:`粘贴并匹配样式`,delete:`删除`,selectAll:`全选`,minimize:`最小化`,zoom:`缩放`,close:`关闭窗口`,front:`全部置于最前`,services:`服务`,hide:`隐藏 Claude`,hideOthers:`隐藏其他`,unhide:`显示全部`};',
    'const i=(e)=>{if(!e||typeof e!==`object`)return e;const n={...e};if(Array.isArray(e.submenu))n.submenu=e.submenu.map(i);if(typeof e.label===`string`){const a=t[e.label]??(/^About (.+)$/.test(e.label)?`关于 ${e.label.slice(6)}`:undefined)??(/^Quit (.+)$/.test(e.label)?`退出 ${e.label.slice(5)}`:undefined);n.label=a??e.label}else if(typeof e.role===`string`&&r[e.role])n.label=r[e.role];return n;};',
